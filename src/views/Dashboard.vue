    ...
    plotOptions: {
        solidgauge: {
            dataLabels: {
                ...
                borderWidth: 0,
                useHTML: true,
                format: '<div style="text-align:center"><span style="font-size:25px;color:' + (isDarkTheme.value ? '#ffffff' : '#374151') + '">{y}%</span></div>'
            }
        }
    },
    series: [
...
<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
        <div class="max-w-7xl mx-auto">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Tableau de bord - Statistiques globales</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-2">Vue d'ensemble des retours clients et taux de satisfaction</p>
            </div>
            ...
            <div v-else-if="error" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
                ...
                <button @click="fetchGlobalStats" class="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Réessayer</button>
            </div>
            ...
            <div v-else-if="stats" class="space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Taux de satisfaction global</h3>
                            ...
                            <div class="relative">
                                <div class="w-48 h-24 relative overflow-hidden">
                                    ...
                                    <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 text-center">
                                        <div class="text-3xl font-bold text-gray-900 dark:text-white">{{ satisfactionRate.toFixed(1) }}%</div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Distribution des avis</h3>
                        <div class="space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="text-right">
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ ((stats.satisfied_count / stats.total_feedbacks) * 100).toFixed(1) }}%</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="text-right">
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ ((stats.neutral_count / stats.total_feedbacks) * 100).toFixed(1) }}%</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="text-right">
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ ((stats.unsatisfied_count / stats.total_feedbacks) * 100).toFixed(1) }}%</div>
                                </div>
                            </div>

                            <div class="space-y-2 mt-6">
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full transition-all duration-1000" :style="{ width: `${(stats.satisfied_count / stats.total_feedbacks) * 100}%` }"></div>
                                </div>
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                    <div class="bg-amber-500 h-2 rounded-full transition-all duration-1000" :style="{ width: `${(stats.neutral_count / stats.total_feedbacks) * 100}%` }"></div>
                                </div>
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                    <div class="bg-red-500 h-2 rounded-full transition-all duration-1000" :style="{ width: `${(stats.unsatisfied_count / stats.total_feedbacks) * 100}%` }"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex justify-center">
                    <button @click="fetchGlobalStats" :disabled="loading" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center">
