        ...
        tooltip: {
            ...
            formatter: function () {
                ...
                this.points.forEach(point => {
                    ...
                    const value = point.series.name.includes('satisfaction')
                        ? `${point.y}%`
...
const averageSatisfaction = computed(() => {
    if (!trendsData.value) return 0;
    const satisfactionSeries = trendsData.value.series.find(s => s.name.includes('satisfaction'));
...
const periodsCount = computed(() => {
    return trendsData.value ? trendsData.value.categories.length : 0;
});
        ...
        <div v-else-if="trendsData" class="space-y-6">
            ...
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                ...
                <div class="overflow-x-auto">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                            <tr v-for="(category, index) in trendsData.categories.slice(0, 10)" :key="index"
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                                    {{trendsData.series.find(s => s.name === 'Totalfeedbacks')?.data[index]?.toLocaleString() || 0 }}
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                        :class="{
                                            'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400': (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) >= 60,
                                            'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400': (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) >= 40 && (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) < 60,
                                            'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400': (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) < 40
                                        }">
                                        {{trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0 }}%
                                    </span>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm">
                                    <span v-if="index > 0" class="inline-flex items-center" :class="{
                                        'text-green-600 dark:text-green-400': (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) > (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index - 1] || 0),
                                        'text-red-600 dark:text-red-400': (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) < (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index - 1] || 0),
                                        'text-gray-600 dark:text-gray-400': (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) === (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index - 1] || 0)
                                    }">
                                        <svg v-if="(trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) > (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index - 1] || 0)"
                                            class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M7 17l9.2-9.2M17 17V7H7"></path>
                                        </svg>
                                        <svg v-else-if="(trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index] || 0) < (trendsData.series.find(s => s.name.includes('satisfaction'))?.data[index - 1] || 0)"
                                            class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div v-if="trendsData.categories.length > 10"
