...
const dominantSentiment = computed(() => {
    ...
    return sentimentData.value.series[0].data.reduce((max, current) => 
...
const createChart = () => {
    try {
        if (chartContainer.value && chartOptions.value && Object.keys(chartOptions.value).length > 0) {
            ...
            // Ajouter un event listener pour les clics (drill-down simulé)
...
// Fonction pour afficher les informations de drill-down
...
// Fonction pour obtenir le libellé de tendance
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
            ...
            <div class="flex items-center gap-4 mt-4 lg:mt-0">
                ...
                <!-- Refresh Button -->
                <button 
                    @click="fetchSentimentDistribution"
                    :disabled="loading"
                ...
                <div class="text-center">
                    <div class="flex items-center justify-center">
                        <span
                            ...
                            :class="getTrendClass(sentimentTrend)"
                        >
                            {{ getTrendIcon(sentimentTrend) }} {{ getTrendLabel(sentimentTrend) }}
                        </span>
                    </div>
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg p-6 border border-blue-200 dark:border-blue-800">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-4 bg-white dark:bg-gray-800 rounded-lg" v-if="sentimentData.series[0].data.find(item => item.name === 'Insatisfait')">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                {{ sentimentData.series[0].data.find(item => item.name === 'Insatisfait').percentage.toFixed(1) }}% 
                                d'insatisfaction représente {{ sentimentData.series[0].data.find(item => item.name === 'Insatisfait').y.toLocaleString() }} 
                        ...
                        <div class="p-4 bg-white dark:bg-gray-800 rounded-lg" v-if="sentimentData.series[0].data.find(item => item.name === 'Neutre')">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                {{ sentimentData.series[0].data.find(item => item.name === 'Neutre').percentage.toFixed(1) }}% 
                                d'utilisateurs neutres peuvent être convertis en clients satisfaits.
                            </p>
                        </div>
                    </div>
                </div>
