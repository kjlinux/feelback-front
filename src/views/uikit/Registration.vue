...
const hasExistingInsuredCountryConflict = computed(() => {
    if (!selectedExistingInsured.value || !selectedCountryExpatriation.value) return false;

    const insuredCountry = selectedExistingInsured.value.country_origin;
    const expatCountry = selectedCountryExpatriation.value.name;

    return insuredCountry === expatCountry || insuredCountry === selectedCountryExpatriation.value.shortName;
});

watch([selectedExistingInsured, selectedCountryExpatriation], ([newInsured, newExpat]) => {
    if (newInsured && newExpat && hasExistingInsuredCountryConflict.value) {
        toast.add({
            severity: 'error',
            summary: 'Erreur de validation',
            detail: "Le pays d'origine de l'assuré et le pays d'expatriation ne peuvent pas être identiques",
            life: 5000
        });
    }
});
...
const canProceed = computed(() => {
    ...
    if (currentStep.value === steps.value.findIndex((s) => s.title === 'Contrat')) {
        const existingInsuredConflict = selectedExistingInsured.value && hasExistingInsuredCountryConflict.value;

        if (hasCountryConflict.value || existingInsuredConflict) {
            toast.add({
                severity: 'error',
                summary: 'Erreur de validation',
                detail: "Le pays d'origine et le pays d'expatriation ne peuvent pas être identiques",
...
const submitForm = async () => {
    try {
        contractData.value.id_products = selectedProduct.value?.id;
        contractData.value.id_countries = selectedCountryExpatriation.value?.id;
        insuredData.value.id_countries = selectedCountryOrigin.value?.id;
        ...
        const contractPayload = {
            ...
            coveragePercentage: parseInt(contractData.value.coveragePercentage) || 0,
            ...
            insured: selectedExistingInsured.value?.id
                ? { id: selectedExistingInsured.value.id }
                : {
        ...
        if (selectedPdfFile.value) {
            ...
            formData.append('media', selectedPdfFile.value);
            formData.append('jsonData', JSON.stringify(contractPayload));

            response = await apiClient.post('/app/contracts', formData, {
                headers: {
            ...
            response = await apiClient.post('/app/contracts', contractPayload, {
                headers: {
        ...
        toast.add({
            ...
            detail: 'Le contrat a été créé avec succès',
            life: 5000
        });
        ...
        let errorMessage = 'Une erreur est survenue lors de la création du contrat';
    ...
    <div class="form-container w-full p-6 bg-white rounded-lg shadow-lg">
        ...
        <TransitionGroup name="fade" mode="out-in" class="w-full">
            ...
            <div v-if="(currentStep === 1 && hasSubscriber === false) || (currentStep === 2 && hasSubscriber === true)" key="step-insured" class="fade-transition w-full">
                <h2 class="text-2xl font-bold mb-6 text-center">{{ showNewInsuredForm ? 'Créer un nouvel assuré' : 'Sélectionner un assuré existant' }}</h2>
            ...
            <div v-if="currentStep === steps.findIndex((s) => s.title === 'Confirmation')" key="step-confirmation" class="fade-transition w-full max-w-4xl mx-auto">
                ...
                <div class="flex justify-center mt-10 mb-6">
                    <button @click="submitForm" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-8 rounded-lg shadow-md transition duration-200 ease-in-out transform hover:-translate-y-1 flex items-center">
...
/* Assurez-vous que les composants InputText et FloatLabel prennent toute la largeur disponible */
:deep(.p-inputtext) {
    width: 100%;
}
